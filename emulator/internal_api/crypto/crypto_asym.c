				     ...
				     void *signature, size_t *signatureLen)
{
	...
	if (rv_mbedtls != 0) {
		...
		OT_LOG_ERR(
		    "Crypto problems: ECDSA Signature (search \"mbedtls\"-error from syslog)");
					...
					size_t digestLen, void *signature, size_t *signatureLen)
{
	...
	if (operation->key_data->key_lenght > *signatureLen) {
		OT_LOG_ERR(
		    "Error (rsa signature): Signature buffer short (provided[%lu]; required[%u])",
				     ...
				     void *signature, size_t signatureLen)
{
	...
	if (operation->key_data->key_lenght != signatureLen) {
		OT_LOG_ERR(
		    "Error (rsa verify): Verify buffer short/big (provided[%lu]; required[%u])",
		    signatureLen, operation->key_data->key_lenght);
		return TEE_ERROR_SHORT_BUFFER;
	}
				      ...
				      void *destData, size_t *destLen)
{
	...
	if (srcLen != operation->key_data->key_lenght) {
		OT_LOG_ERR(
		    "Error (rsa decrypt): srcLen too big/small (provided[%lu]; required[%u])",
				    ...
				    void *signature, size_t *signatureLen)
{
	...
	if (operation == NULL) {
		...
		OT_LOG_ERR(
		    "TEE_AsymmetricSignDigest panics due operation mode is not TEE_MODE_SIGN");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
				      ...
				      void *signature, size_t signatureLen)
{
	...
	if (operation == NULL) {
		...
		OT_LOG_ERR(
		    "TEE_AsymmetricVerifyDigest panics due operation mode not TEE_MODE_VERIFY");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	} else if (operation->operation_info.operationClass != TEE_OPERATION_ASYMMETRIC_SIGNATURE) {
